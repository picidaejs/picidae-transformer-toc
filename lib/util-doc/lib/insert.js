...
function insert(node, parent, tight) {
  ...
  node.children = node.children.slice().map(function (node) {
    if (node.type === 'link') {
  ...
  if (node.depth === 1) {
    ...
    if (node.children[0] && node.children[0].type === LINK) {
      pushItem = node.children[0]
      pushItem.children = pushItem.children || []
      pushItem.children = pushItem.children.concat(node.children.slice(1))
    }
    else {
      pushItem = {
        type: LINK,
